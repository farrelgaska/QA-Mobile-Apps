  ...
  constructor() {
    ...
    this.client = {
      query: async (text, parameters = []) => {
        ...
        if (text.includes('from public.qc_reports where id')) {
          if (parameters[0].includes('MISSING')) return { rows: [], rowCount: 0 };
          return {
            rows: [{
              id: parameters[0],
              type: 'MATERIAL',
...
test('PostgreSQL aggregate deletes commit after deleting only the parent row', async () => {
  ...
  assert.equal(
    reportPool.commands.some(command => command.startsWith('select id, type, template_id')),
    true
  );
  assert.deepEqual(
    reportPool.commands.filter(command => command.startsWith('delete from public.')),
    ['delete from public.qc_reports where id']
  );
