...
test('non-Supabase providers cannot initialize the Supabase adapter in production', () => {
  for (const storageProvider of ['s3', 'gcs']) {
    ...
    assert.throws(getStorage, /QC evidence upload requires STORAGE_PROVIDER=supabase in production/);
  }
});
...
test('local fallback createSignedUrls uses mock-storage for existing files', async () => {
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'qc-local-'));
  // Monkey patch path.join to redirect to tempDir for .local-storage check
  const originalJoin = path.join;
  path.join = (...args) => {
    if (args.includes('../../.local-storage/qc-evidence')) return tempDir;
    return originalJoin(...args);
  };
  
  try {
    const testPath = 'reports/mock/test.jpg';
    const absolutePath = originalJoin(tempDir, testPath);
    await fs.promises.mkdir(path.dirname(absolutePath), { recursive: true });
    await fs.promises.writeFile(absolutePath, Buffer.from('test'));

    const getStorage = createQCEvidenceStorageProvider({
      config: { STORAGE_PROVIDER: 'local', API_BASE_URL: 'http://test.local' },
      clientFactory: () => assert.fail()
    });

    const storage = getStorage();
    const result = await storage.createSignedUrls([testPath]);

    assert.deepEqual(result, {
      signedUrls: [{
        object_path: testPath,
        signed_url: 'http://test.local/mock-storage/reports/mock/test.jpg',
        expires_in: 3600
      }],
      failedPaths: []
    });
...
test('local fallback createSignedUrls places missing files in failedPaths if no Supabase config', async () => {
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'qc-local-'));
  const originalJoin = path.join;
  path.join = (...args) => {
    if (args.includes('../../.local-storage/qc-evidence')) return tempDir;
  ...
  try {
    ...
    const storage = getStorage();
    const result = await storage.createSignedUrls(['reports/missing/test.jpg']);

    assert.deepEqual(result, {
      signedUrls: [],
      failedPaths: ['reports/missing/test.jpg']
    });
  } finally {
    path.join = originalJoin;
    await fs.promises.rm(tempDir, { recursive: true, force: true });
...
test('local fallback createSignedUrls uses Supabase for missing files if config exists', async () => {
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'qc-local-'));
  const originalJoin = path.join;
  path.join = (...args) => {
    if (args.includes('../../.local-storage/qc-evidence')) return tempDir;
  ...
  try {
        ...
        storage: {
          from: () => ({
            createSignedUrls: async (paths) => ({
              data: paths.map(p => ({ path: p, signedUrl: `https://test.supabase/${p}` })),
              error: null
            })
          })
        }
    ...
    const storage = getStorage();
    const result = await storage.createSignedUrls(['reports/remote/test.jpg']);

    assert.deepEqual(result, {
      signedUrls: [{
        object_path: 'reports/remote/test.jpg',
        signed_url: 'https://test.supabase/reports/remote/test.jpg',
        expires_in: 3600
      }],
      failedPaths: []
    });
