...
export function mapToSharedReport(report: any): QCReport {
  ...
  const checklist_items: SharedChecklistItem[] = rawItems.map((item: any) => {
    ...
    const rawStaffEvaluation = (
      item.staff_evaluation || item.evaluation_status || 'NOT_EVALUATED'
    ).toUpperCase();
    const staffEvaluation: ParameterEvaluationStatus = (
      rawStaffEvaluation === 'WITHIN_STANDARD' ||
      rawStaffEvaluation === 'OUT_OF_STANDARD'
    ) ? rawStaffEvaluation : 'NOT_EVALUATED';
    ...
    return {
      ...
      staff_evaluation: staffEvaluation,
  ...
  return {
    ...
    checklistItems: checklist_items.map(item => ({
      ...
      staffEvaluation: item.staff_evaluation,
      unit: item.unit,
