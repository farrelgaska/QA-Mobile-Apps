  ...
  return (
    <div className="flex min-w-[280px] max-w-md flex-col gap-2 rounded-lg border border-gray-200 bg-gray-50/60 p-2 sm:flex-row">
      <button
        type="button"
        onClick={onPreview}
        className="relative h-20 w-full flex-shrink-0 overflow-hidden rounded-md border border-gray-200 bg-white hover:border-[#006B5A] sm:w-20 group"
        aria-label={`Buka ${alt}`}
      >
      ...
      <div className="min-w-0 flex-1 space-y-1 text-[11px] leading-snug text-gray-600 text-left">
        ...
        {metadata.mapUrl ? (
          ...
          <span className="text-gray-400 italic text-[10px]">Maps tidak tersedia</span>
        )}
