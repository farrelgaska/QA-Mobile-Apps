      ...
      <Table>
        <TableHeader>
          <TableRow>
            ...
            <TableCell isHeader>Status Staff</TableCell>
            <TableCell isHeader>Evaluasi Admin</TableCell>
              ...
              <TableRow key={item.id} className="align-middle hover:bg-gray-50/55 transition-colors duration-150">
                ...
                <TableCell>
                  <span className={`inline-flex items-center rounded-full px-2 py-1 text-xs font-semibold ${
                    item.staffEvaluation === 'WITHIN_STANDARD'
                      ? 'bg-emerald-50 text-emerald-700'
                      : item.staffEvaluation === 'OUT_OF_STANDARD'
                ...
                <TableCell>
                  <div className="flex flex-wrap gap-1.5">
                    {item.photoUrls && item.photoUrls.length > 0 ? (
                      item.photoUrls.map((path, i) => {
                        const displayUrl = evidenceDisplayUrls?.[path] ?? path;
                        return (
                          <EvidenceCard
                            key={i}
                            objectPath={path}
