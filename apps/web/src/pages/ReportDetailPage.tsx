...
const workQcIcons = [MapPin, ClipboardList, MapPin, User];
...
export const ReportDetailPage: React.FC = () => {
  ...
  const reportAdminNote = report?.adminNote;
  ...
  useEffect(() => {
    if (reportAdminNote !== undefined) {
      setAdminFeedback(reportAdminNote);
    }
  }, [reportAdminNote]);
            ...
            <Card title="Data QC Pekerjaan">
              <CardContent className="space-y-4 pt-3">
                {workQcDataRows(report).map((row, index) => {
                  const Icon = workQcIcons[index] ?? Info;
          ...
          <Card title="Evaluasi Parameter Standar QC">
            <CardContent className="pt-3">
              ...
              <ChecklistEvaluationTable
                ...
                evidenceDisplayUrls={report.evidenceDisplayUrls}
                isEditable={isEditable}
